...
- [x] Seed 7 departments (YCDC-aligned real names, no "Engineering Department" prefix; renamed in place from the original 6 + one new Drainage Management row)
- [x] Seed 14 default categories with department mapping (expanded from 6)
...
- [x] Pseudo-anonymous reporting — `reports.is_anonymous` column + optional `CreateReportDTO.isAnonymous`; `GET /api/reports/public` (paged public feed, active statuses, newest first) returns a new `PublicReportDTO` whose `ReportMapper.toPublicDTO()` strips `reporterName`/`reporterAvatarUrl` when anonymous. `ReportDTO` gained `anonymous` (ADMIN/STAFF keep the real reporter). Tests: `ReportServiceTest.getPublicFeed_queriesOnlyTheActiveMiddleStatuses`, `ReportControllerIntegrationTest` — anonymous create persists the flag while admin still sees the reporter, public feed masks anonymous rows + excludes RESOLVED, 401 unauthenticated. See Decisions.
- [~] `DuplicateDetectionService` (`report/service/`) — proximity-based duplicate detection (problem #6 in `project-overview.md`; not an AI feature, see Decision D20): same category + non-terminal status + within 100m, via a `ReportRepository.findOpenCandidatesNear()` bounding-box pre-filter (`±0.000898°` lat, `±(100/(111320·cos(lat)))°` lng) then a precise Haversine pass in Java, sorted ascending, capped at 5. `ReportService.submitReport()` now sits in front of `createReport()`: no `confirmDuplicateOfId`/`forceCreate` + matches found → `200` + `DuplicateCheckResultDTO`, nothing persisted; `confirmDuplicateOfId` set → new `ReportConfirmation` row (`report_confirmations`, unique per `(report_id, citizen_id)`, pre-checked the same way `FeedbackService` pre-checks `existsByReportId` — 409 on a repeat) + `+3` points (`PointReason.CONFIRMATION_GIVEN`, new enum value) instead of a new report; `forceCreate: true` → skips the check, creates normally, sets the new `reports.duplicate_checked` column `true`. `createReport()` itself is unchanged in behavior (still unconditional) — kept as the method `submitReport()` delegates to internally (via a new 4-arg overload carrying the `duplicateChecked` flag) and that existing tests still exercise directly. `POST /api/reports` controller method now returns `ResponseEntity<?>` (`ReportSubmissionResultDTO` picks the status/body shape) since it can return three different response shapes; `api-standards.md` documents all three. Marked in-progress, not done, for the same KI-03 reason as the `PriorityService` line above — code and tests are written but `mvn test` could not be run this session.
...
- [~] Duplicate-detection tests (untested by `mvn test` this session, KI-03): `DuplicateDetectionServiceTest` (unit) — in-radius candidate returned with distance, a bounding-box-corner candidate ~130m away correctly excluded by the precise Haversine pass despite passing the SQL pre-filter, terminal statuses (`RESOLVED`/`CLOSED`/`REJECTED`) passed as the repository's excluded-status argument, sort-ascending-and-cap-at-5 over 6 candidates. `ReportServiceTest` (unit) — `submitReport()`'s three branches (created/duplicates-found/confirmed), `forceCreate` sets `duplicateChecked`, confirming the same report twice throws `DuplicateResourceException`. `ReportControllerIntegrationTest` (integration, real H2) — within-100m → `200` + `possibleDuplicates` and no report persisted, >100m apart → normal `201`, same location different category → `201`, an existing `RESOLVED` report at the same spot never offered → `201`, `forceCreate` → `201` with `duplicateChecked=true` verified via a DB re-fetch, confirm-then-reconfirm → `200` then `409`. (Also had to move 4 pre-existing `ReportControllerIntegrationTest` create-flow tests off the shared `16.8409000/96.1735000` fixture coordinate onto their own dedicated locations — this test class's `@SpringBootTest` context and H2 database are shared and never rolled back across test methods/classes, so those tests would otherwise now trip the new duplicate check against each other and against unrelated fixture reports in `ReportMapControllerIntegrationTest`/`ReportWorkflowControllerIntegrationTest`/`DashboardControllerIntegrationTest` that reuse the same coordinate; those other three files were left untouched since none of them call the create endpoint themselves, so they can't trigger the new check.)
...
- [x] Citizen home "What's happening in Yangon?" feed — reads `GET /api/reports/public`; renders "Anonymous Citizen" (mask icon) vs. reporter avatar + name per `anonymous`. Report form has a "Submit anonymously" `Switch` (default off) → `isAnonymous` in the `data` part; console `OverviewTab` + citizen report detail show an "Anonymous" badge while ADMIN/STAFF still see the name.
...
- [~] `PriorityService` (`report/service/PriorityService.java`) — computes priority automatically in `ReportWorkflowService.approve()` from `category.severityWeight` (1–5, seeded per category in `DataSeeder`), a duplicate-count bonus (parameter still hardcoded `0` at the `approve()` call site — see the follow-up note below now that `DuplicateDetectionService`/`report_confirmations` exist), and a report-age bonus; writes both `report.priority` and the new `reports.priority_score` column. `categories.severity_weight` and `reports.priority_score` added to `database-schema.md` in the same change. Marked in-progress, not done, because `mvn test` could not be run this session (see Known Issues KI-03) — code and tests are written but unverified by an actual test run.
- [ ] Follow-up (not built this session): wire `PriorityService`'s `duplicateCount` parameter to a real count — e.g. `ReportConfirmationRepository`'s confirmation rows for the report being approved — now that Phase 4's duplicate-detection feature actually produces that data. Out of scope for the session that added duplicate detection (task brief didn't ask for it); still hardcoded to `0` in `ReportWorkflowService.approve()`.
- [x] History endpoint
...
### Audit Trail (added 2026-09-10)
- [x] `audit_logs` table + `AuditLog`/`AuditAction` (`audit/entity/`), `AuditLogRepository` (paged `search`, `findDistinctActors`)
- [x] `AuditLogWriter` (`REQUIRES_NEW`) + `AuditService.record()` — best-effort, actor resolved from the JWT security context, never throws, never rolls back the audited action
- [x] `AuditService.record()` wired into `UserService` (approve/reject/suspend/createStaff), `DepartmentService` (create/update/delete), `CategoryService` (create/update/delete), `ReportWorkflowService` (approve/reject/changeStatus/updatePriority), `ReportAssignmentService` (assign)
- [x] `GET /api/audit-logs` + `GET /api/audit-logs/actors` (`AuditLogController`, ADMIN only, `@Tag`)
- [x] `AuditServiceTest` (6 cases: actor stamping, system fallback, failure-swallow, text clamp, search filter binding); the 5 touched service unit tests updated with `@Mock AuditService`
- [x] Frontend: `auditLogsApi`, `useAuditLog` (server-side filters + pagination + 30 s silent refresh), `AuditLogsPage` rewritten to the real feed; deleted the in-memory `auditLogStore` and the client-side `addAuditEntry` calls in `useAdminApprovals`
- [ ] Postman: `audit-logs` requests not yet added to `docs/postman/SCIRS.postman_collection.json`
...
| 2026-09-05 | Added proximity-based duplicate detection: `DuplicateDetectionService`, `ReportConfirmation` entity/repository/table, `ReportService.submitReport()` orchestrating the confirm/force-create round trip, `PointReason.CONFIRMATION_GIVEN` (+3), `reports.duplicate_checked` column; `POST /api/reports` controller method changed from `ResponseEntity<ReportDTO>` to `ResponseEntity<?>` since it can now return a `ReportDTO` (201/200) or a `DuplicateCheckResultDTO` (200) | Problem #6 in `project-overview.md`'s problem statement ("duplicate reports... cause redundant site visits"); see D20. `List<PossibleDuplicateDTO> findPossibleDuplicates(...)` was used instead of the task brief's suggested `List<ReportDTO>` signature because the response needs `distanceMeters`/`thumbnailUrl`, which `ReportDTO` doesn't carry — a deliberate, justified deviation from the "e.g." example, not a missed requirement. `ReportService.createReport(dto, images, citizenId)` (the pre-existing 3-arg method, and all its existing tests) is untouched in behavior; a new 4-arg overload carrying a `duplicateChecked` boolean was added instead of changing its signature, and `submitReport()` calls that overload internally — this avoided rewriting `ReportServiceTest`'s and `ReportControllerIntegrationTest`'s pre-existing `createReport`-focused assertions, which all still call/expect the original 3-arg happy path. The task brief's draft D13 numbering collided with the already-used Supabase Storage decision, so this landed as D20 in `project-overview.md` instead — see that entry's parenthetical for why. |
...
| 2026-09-09 | Renamed the 6 seeded departments to their real YCDC-aligned names with the "Engineering Department" wrapper dropped (`Roads & Bridges Department`, `Buildings Department`, `Water & Sanitation Department`, `Urban Environmental Conservation & Cleansing Department`, `Playgrounds, Parks & Gardens Department`, `Yangon Electricity Supply Corporation (YESC)`), added a 7th — `Drainage Management Department` — and expanded 6 categories to 14 (two per department) | User asked for a realistic (non-placeholder) department/category taxonomy for the graded demo, explicitly to drop the generic "Engineering Department" grouping shown in their reference table. `DataSeeder.seedDepartments()`/`seedCategories()` were rewritten to rename a matching legacy-named row in place (`legacyName` field on the seed record) rather than insert a duplicate, so existing `department_id`/`category_id` foreign keys on reports, staff, and status history survive a redeploy against a database seeded before this change. `MockDataSeeder`'s 7 staff specs and 30 report specs were remapped to the new names (categories resolved by name at runtime, so this had to land in the same commit or it would NPE); 3 of the 30 demo reports were rewritten (not just renamed) so all 14 categories — including the new footpath/construction/water-supply/culvert/flooding/dumping/tree/cable ones — have at least one seeded report. Demo photo URLs (`picsum.photos` placeholders) were intentionally left unchanged this pass — the user asked to defer curated real-photo sourcing and do the rest first; see `docs/bug-log.md` / the plan file for the follow-up. ~15 test classes that resolve seeded rows by literal name (`findByName("Roads")` etc.) were updated to the new literals. |
| 2026-09-10 | Realistic demo photos for seeded reports. 13 category-named "before" photos bundled at `backend/src/main/resources/seed-images/`, served read-only from the jar at `/seed-images/**` (WebConfig + SecurityConfig permitAll); `MockDataSeeder.PHOTO_BY_CATEGORY` + `photoUrlFor()` prefix `app.public-base-url` (default `http://localhost:8080`) so each URL is absolute and renders as a plain `<img src>` with no frontend change. Follow-up: 8 "after"/resolution photos (Pexels licence, provenance in `seed-images/CREDITS.md`) via `RESOLUTION_PHOTO_BY_CATEGORY` + `resolutionPhotoUrlFor()` for the categories with a RESOLVED/CLOSED demo report (streetlight, road, water supply, garbage, playground, public building, power, drain); others fall back to the "before" photo. A fresh seed picks these up automatically; `docs/seed-photos-update.sql` repoints an already-seeded DB, `docs/reseed-demo.sql` wipes + rebuilds the demo set. | Replaces the `picsum.photos` random-image placeholders that made the graded demo read as mock data. Photos bundled rather than hot-linked because there is no Supabase key in this environment and the frontend has no image-URL resolver/proxy. Resolution photos were split out so a RESOLVED report shows a genuine before/after pair instead of the same shot twice. |
| 2026-09-10 | Audit trail made real. New append-only `audit_logs` table + `audit` module (`AuditLog`, `AuditAction` 15-value enum, `AuditLogRepository`, `AuditLogMapper`, `AuditLogController` — `GET /api/audit-logs` + `/actors`, ADMIN only). `AuditService.record(action, targetType, targetId, targetLabel, details)` is called from `UserService`, `DepartmentService`, `CategoryService`, `ReportWorkflowService`, `ReportAssignmentService` at the end of each state-changing method. The frontend `admin-audit` feature was rewritten off its in-memory `auditLogStore` onto the real paged endpoint; the client-side `addAuditEntry` calls in `useAdminApprovals` were removed. | The Audit Logs page shipped in the Phase-7 frontend scaffold as a pure in-memory store (`auditLogStore.js`) seeded with fake rows — nothing survived a refresh and staff/report actions never appeared. **Actor is resolved from the JWT security context inside `AuditService`, not threaded through every service signature** — `UserService.approve(id)` etc. never took a `CurrentUser`, and adding one to ~10 methods + their controllers + their tests was disproportionate. **`AuditLogWriter.write()` is `@Transactional(REQUIRES_NEW)` and `AuditService.record()` swallows any failure**: the audit write is isolated from the caller's transaction so an audit failure can never mark the audited operation rollback-only (the classic "caught the exception but the tx is already doomed" trap), and never blocks the operation. Actor/target are denormalised snapshots (id + label string), matching the plain-FK-id convention `users.department_id` already follows, so a row stays readable after a rename/deactivation. The 5 touched service unit tests gained `@Mock AuditService`; a new `AuditServiceTest` covers actor stamping, the system fallback, failure-swallowing, text clamping, and filter binding. The pre-existing `@SpringBootTest` integration suite is red on this machine for an unrelated reason (H2 rejects Hibernate's `insert … returning` as unrecognised syntax); 163 unit tests pass, the integration tests need the Postgres-backed profile to run. |
| 2026-09-10 | Pseudo-anonymous reporting — "Anonymous to the Public, Known to the System". New `reports.is_anonymous` column (`@ColumnDefault("false")` so Hibernate `ddl-auto=update` can add a `NOT NULL` column to the existing Supabase table). Masking is enforced **server-side, before serialization**: a dedicated `PublicReportDTO` + `GET /api/reports/public` feed endpoint (paged, `ASSIGNED`/`IN_PROGRESS` only, newest first) whose mapper nulls `reporterName`/`reporterAvatarUrl` for anonymous rows; the identity never leaves the API. `ReportDTO` (ADMIN/STAFF only) keeps `reporterName`/`reporterId` and adds `anonymous` so consoles can badge it. Scoring/leaderboard untouched (points are private to the reporter's own score page, never tied to a report in public views). | The citizen "What's happening in Yangon?" feed had no way to show other citizens' reports at all — `ReportService.assertCanView` blocks a citizen from `GET /reports/{id}` on a report they don't own, so the old frontend per-pin enrichment silently fell back to pin-only data. A public feed endpoint was needed regardless; folding anonymity into it means the mask is applied in one place the client can't bypass, rather than trusting the client to hide a name it was still sent. Toggle defaults **off** (attributed) — the civic-tech norm (NYC 311 / FixMyStreet); user confirmed. Label is a generic "Anonymous Citizen" (no per-user pseudonym — a stable pseudonym lets readers correlate a person's reports). |
