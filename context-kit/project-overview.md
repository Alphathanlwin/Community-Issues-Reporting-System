...
| D9 | `POST /api/reports` (create-report only) was built before Phase 3 (user approval queue/staff creation) was started | A session task explicitly scoped work to this one endpoint. It only depends on the `User.accountStatus` field from Phase 1, not on any Phase 3 endpoint. Phase 3 remains required before Phase 4 is considered complete — see `progress-tracker.md` § Decisions for the full rationale. |
| D10 | `Report`/`ReportImage` were modelled with their full `database-schema.md` column set (including Phase-5-flavoured columns like `assignedStaffId`, `approvedAt`, `resolvedAt`) even though only report creation is implemented so far | The schema doc is the source of truth for the table shape; building a partial entity now would force a breaking rewrite once Phase 5 (workflow) starts. No workflow *logic* was added — only nullable, unused-for-now columns. |
| D11 | Phase 7 (Notifications, Feedback, Gamification) was built on a deliberately minimal slice of Phase 3 and Phase 5, not those phases in full, and `/frontend` was scaffolded from scratch with only the routes needed to reach the Phase 7 UI | Explicit user override of `build-plan.md`'s sequencing rule for this session, scoped to exactly what Phase 7's triggers depend on (account approval so users exist; approve/reject/status-change/history so a report can reach `RESOLVED`). Reassignment, priority, comments, registration, report submission, the map, and the full consoles remain for their own phases — see `progress-tracker.md` § Decisions for the itemised breakdown. |
