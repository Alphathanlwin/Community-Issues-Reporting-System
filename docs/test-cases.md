...
| FBK-01 | Feedback | Feedback on a non-resolved report | POST `/api/feedback` for a report still `IN_PROGRESS` | 400 `BusinessRuleException` | As expected | Pass | 2026-08-31 |
| FBK-02 | Feedback | Duplicate feedback on the same report | POST `/api/feedback` twice for the same `RESOLVED` report | 409 `DuplicateResourceException` on the second call | As expected | Pass | 2026-08-31 |
| FBK-03 | Feedback | Feedback from a non-reporter | POST `/api/feedback` as a citizen who did not file the report | 403 | As expected | Pass | 2026-08-31 |
| FBK-04 | Feedback | Feedback on own resolved report | POST `/api/feedback` with `rating=5` by the reporter, report `RESOLVED` | 201, feedback persisted, `+5` points awarded exactly once | As expected | Pass | 2026-08-31 |
| FBK-05 | Feedback | Read feedback for a report | GET `/api/feedback/{reportId}` as ADMIN, STAFF, or the reporting CITIZEN | 200 | As expected | Pass | 2026-08-31 |
| NOTIF-01 | Notification | New report notifies every admin | Citizen submits a report with two ADMIN accounts seeded | Two `NEW_REPORT` notifications created, one per admin, plus an email to each | As expected | Pass | 2026-08-31 |
| NOTIF-02 | Notification | Status change to RESOLVED notifies the reporter | Staff moves a report to `RESOLVED` | `REPORT_COMPLETED` notification created for the reporter | As expected | Pass | 2026-08-31 |
| NOTIF-03 | Notification | Mark a notification read (not owner) | PATCH `/api/notifications/{id}/read` with a different citizen's JWT | 403 | As expected | Pass | 2026-08-31 |
| NOTIF-04 | Notification | Mark a notification read (owner) | PATCH `/api/notifications/{id}/read` with the recipient's own JWT | 200, `isRead=true` | As expected | Pass | 2026-08-31 |
| NOTIF-05 | Notification | Unread count | GET `/api/notifications/unread-count` with 3 unread seeded | 200, `{"count":3}` | As expected | Pass | 2026-08-31 |
| NOTIF-06 | Notification | List own notifications | GET `/api/notifications` as any authenticated role | 200, newest first, only the caller's own rows | As expected | Pass | 2026-08-31 |
| SCORE-01 | Score | Award points, first time for report+reason | `ScoreService.award(citizen, REPORT_APPROVED, report)` on a fresh `(user, report, reason)` triple | `+10` transaction row created, `user.scorePoints` incremented | As expected | Pass | 2026-08-31 |
| SCORE-02 | Score | Award points, same report+reason again | Call `award()` twice with the same `(user, report, reason)` triple | Second call is a no-op — no duplicate transaction, `scorePoints` unchanged | As expected | Pass | 2026-08-31 |
| SCORE-03 | Score | Own score + history | GET `/api/score/me` with a CITIZEN JWT | 200, total matches the sum of that citizen's point transactions | As expected | Pass | 2026-08-31 |
| LB-01 | Leaderboard | Leaderboard order | GET `/api/leaderboard` with citizens seeded at different `scorePoints` | 200, ranked highest-`scorePoints`-first, ties broken by earliest `createdAt` | As expected | Pass | 2026-08-31 |
| LB-02 | Leaderboard | Leaderboard limit | GET `/api/leaderboard?limit=20` | 200, at most 20 entries returned | As expected | Pass | 2026-08-31 |
| UPL-01 | Upload | Non-image file rejected | `LocalStorageService.store()` with `contentType="application/pdf"` | `FileStorageException` — actually surfaces as `400` via `GlobalExceptionHandler`, not the `415` `api-standards.md` documents for this row (see KI-02 in `progress-tracker.md` § Known Issues) | As expected (given the current mapping) | Pass | 2026-08-31 |
| UPL-02 | Upload | Stored filename differs from uploaded filename | `LocalStorageService.store()` with a valid JPEG named `my-pothole-photo.jpg` | Returned URL's filename is a generated UUID, not `my-pothole-photo.jpg` | As expected | Pass | 2026-08-31 |
| ANON-01 | Report | Anonymous submission persists the flag; admin still sees the reporter | POST `/api/reports` multipart with `"isAnonymous": true` in the `data` part, as an APPROVED CITIZEN | 201, response `anonymous=true` and `reporterName` still populated; `reports.is_anonymous` is `true` in the DB | As expected | Pass | 2026-09-10 |
| ANON-02 | Report | Public feed masks anonymous reporters, keeps attributed ones, excludes RESOLVED | GET `/api/reports/public` as CITIZEN with one anonymous `IN_PROGRESS`, one attributed `ASSIGNED`, and one `RESOLVED` report seeded | 200, anonymous row has `anonymous=true` + `reporterName`/`reporterAvatarUrl` `null` but a real `title`; attributed row has `anonymous=false` + `reporterName`; the RESOLVED report is absent | As expected | Pass | 2026-09-10 |
| ANON-03 | Report | Public feed requires authentication | GET `/api/reports/public` with no `Authorization` header | 401 | As expected | Pass | 2026-09-10 |
| ANON-04 | Report | Public feed queries only active middle statuses | `ReportService.getPublicFeed()` (unit) | Delegates to `ReportRepository.findByStatusInOrderByCreatedAtDesc([ASSIGNED, IN_PROGRESS], pageable)` — never `PENDING_APPROVAL`/`REJECTED`/`RESOLVED`/`CLOSED` | As expected | Pass | 2026-09-10 |
| AUDIT-01 | Audit | Approving a citizen writes an audit row | ADMIN calls PATCH `/api/users/{id}/approve`; then GET `/api/audit-logs` | 200; newest row is `action=USER_APPROVED`, `targetType=USER`, `actorName`/`actorEmail` = the admin, `details="PENDING → APPROVED"` | Not run — `@SpringBootTest` suite blocked on this machine (H2 vs Hibernate `insert … returning id`, pre-existing) | Blocked | 2026-09-10 |
| AUDIT-02 | Audit | Report transitions are recorded with the operator and state change | STAFF moves a report `ASSIGNED → IN_PROGRESS` via PATCH `/api/reports/{id}/status` | An `action=REPORT_STATUS_CHANGED` row with the staff member as actor, `targetLabel="RPT-… — <title>"`, `details="ASSIGNED → IN_PROGRESS"` | Not run — see AUDIT-01 | Blocked | 2026-09-10 |
| AUDIT-03 | Audit | Feed is ADMIN-only | GET `/api/audit-logs` with a STAFF JWT, then a CITIZEN JWT | 403 for both | Not run — see AUDIT-01 (`@PreAuthorize("hasRole('ADMIN')")` on the controller) | Blocked | 2026-09-10 |
| AUDIT-04 | Audit | Filters — action, actor, search, date range | GET `/api/audit-logs?action=REPORT_APPROVED&actorId=1&search=streetlight&startDate=2026-09-01` | 200, only rows matching every supplied filter; `search` matches actor/target/details case-insensitively | Filter binding covered by `AuditServiceTest`; end-to-end not run — see AUDIT-01 | Partial | 2026-09-10 |
| AUDIT-05 | Audit | A failing audit write never breaks the audited action | `AuditServiceTest`: `record()` with `AuditLogWriter.write()` stubbed to throw | The exception is swallowed and logged; the caller returns normally | As expected | Pass | 2026-09-10 |
| AUDIT-06 | Audit | Actor falls back to "System" with no security context | `AuditServiceTest`: `record()` called with no `Authentication` set | Row persisted with `actorId=null`, `actorName="System"`; no `UserRepository` lookup | As expected | Pass | 2026-09-10 |
| AUDIT-07 | Audit | Touched service tests still green with the new dependency | `./mvnw test -Dtest='UserServiceTest,DepartmentServiceTest,CategoryServiceTest,ReportWorkflowServiceTest,ReportAssignmentServiceTest,AuditServiceTest'` | 57/57 pass (51 existing + 6 new) | As expected | Pass | 2026-09-10 |
